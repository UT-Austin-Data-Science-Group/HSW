...
import models
import datasets
from functions import train_hsw, validate, LinearLrDecay, load_params, copy_params
...
def main():
    ...
    if args.load_path:
        ...
        # create new log dir
        args.path_helper = set_log_dir(args.dataset.lower()+'_logs', 'hsw_Ls{}'.format(args.Ls))
        logger = create_logger(args.path_helper['log_path'])
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        train_hsw(args, gen_net, dis_net, slicer, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
