...
import models
import datasets
from functions import train_maxhsw, validate, LinearLrDecay, load_params, copy_params
...
import numpy as np
from slicers import Hierarhcial_Slicer
...
def main():
    ...
    if (args.dataset.lower() == 'stl10'):
        ...
        feat_size = 8192
    Ls=args.Ls
    Ls = [int(L) for L in Ls.split(',')]
    slicer = Hierarhcial_Slicer(d=feat_size, Ls=Ls).cuda()
    # set optimizer
    ...
    slicer_scheduler = LinearLrDecay(dis_optimizer, args.s_lr, 0.0, 0, args.max_iter * args.n_critic)
    # set up data_loader
    ...
    if args.dataset.lower() == 'cifar10':
        ...
        fid_stat = 'fid_stat/fid_stats_tinyimagenet.npz'
    else:
    ...
    if args.load_path:
        ...
        slicer.load_state_dict(checkpoint['slicer_state_dict'])
        gen_optimizer.load_state_dict(checkpoint['gen_optimizer'])
        dis_optimizer.load_state_dict(checkpoint['dis_optimizer'])
        slicer_optimizer.load_state_dict(checkpoint['slicer_optimizer'])
        ...
        # create new log dir
        args.path_helper = set_log_dir(args.dataset.lower()+'_logs', 'maxhsw_Ls{}_slr{}_nlr{}'.format(args.Ls,args.s_lr,args.s_max_iter))
        logger = create_logger(args.path_helper['log_path'])
    ...
    fids=[]
    iscores=[]
    lr_schedulers = (gen_scheduler, dis_scheduler,slicer_scheduler) if args.lr_decay else None
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        train_maxhsw(args, gen_net, dis_net, slicer, gen_optimizer, dis_optimizer,slicer_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
